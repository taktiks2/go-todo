...
| **シグナル名のログ** | `slog.Info("shutting down", "cause", context.Cause(ctx))` | **Go 1.26 の新挙動**: `NotifyContext` はシグナル起因のキャンセル時、`context.Cause` にどのシグナルかを示すエラーを入れる。SIGTERM（Cloud Run のスケールダウン）か `os.Interrupt`（ローカルの Ctrl-C）かがログで区別できる。**`errors.Is(cause, context.Canceled)` の結果は Go のパッチバージョンで変わる**（実測: go1.26.0 は false、go1.26.5 は true。`signalError.Is` が 1.26 系の途中で入ったため）。**`Cause` は制御フローの判定に使わずログ専用にする。** `go.mod` の go directive は `1.26.5` にして環境差を塞いだ |
